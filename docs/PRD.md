...
The agent chain editor is the visual pipeline builder for Insomniac. It is built on ReactFlow (`@xyflow/react`, fully MIT licensed) and carries an n8n-inspired aesthetic: a dark infinite canvas with typed nodes, curved bezier edges, and a slide-in node inspector panel. It is the primary way developers compose, inspect, and customise multi-agent pipelines.

**ReactFlow licence:** Fully MIT including the Pro Platform. No licence friction for open source or commercial use.

#### Canvas

- Infinite canvas rendered by ReactFlow with pan (drag background) and zoom (scroll)
- Dark theme: `#0a0d13` canvas background, `#111827` node cards, `#1e2a3a` borders
- Grid dot pattern at low opacity for spatial orientation
- Double-click empty canvas to open the Add Node menu at cursor position
- Right-click empty canvas: context menu with Add Node options
- Keyboard: `Delete` or `Backspace` to remove selected node or edge; `Escape` to deselect

#### Node types

Each node type has a fixed colour dot and default system prompt. Built-in types:

| Type | Colour | Default role |
|---|---|---|
| Trigger | Cyan `#06b6d4` | Receives user intent, parses into structured task definition |
| Prototyper | Violet `#8b5cf6` | Rapid scaffolding, component tree, stub files |
| Builder | Indigo `#6366f1` | Feature implementation via Claude Code tools |
| Tester | Emerald `#10b981` | Unit, integration, and Playwright E2E tests |
| Reviewer | Amber `#f59e0b` | Code review, quality gates, PR analysis |
| Auditor | Red `#ef4444` | Security scan, OWASP Top 10, dependency audit |

Custom node types can be added. A custom node inherits the Builder type's defaults and can be renamed and re-coloured.

#### Node anatomy

Each node card on the canvas displays:
- Coloured type dot + node name (bold, 11px) + model name (muted, 10px) in the header
- First 72 characters of the active system prompt as a preview in the body
- Status badge in the footer: `pending` / `running` / `done` / `error`
- **Open** button — opens the node inspector panel
- **✕** button — deletes the node and all connected edges
- Input port (left edge) and output port (right edge) — circular handles for connecting edges

#### Edges

- Edges are smooth bezier curves drawn between output and input ports
- Connection: click an output port → drag → release on an input port. A dashed preview curve follows the cursor during drag.
- Edge types encode execution condition, shown by colour:
  - `always` — gray `#374151`
  - `on-success` — emerald `#10b981`
  - `on-failure` — red `#ef4444`
  - `on-decision` — amber `#f59e0b`
  - `active` (currently executing) — indigo `#6366f1` with animation
- Hovering an edge reveals a `×` delete button at the midpoint
- Edges cannot form cycles — the editor validates and rejects circular connections

#### Node inspector panel

Clicking "Open" on any node slides in a 340px panel from the right side of the canvas. The panel does not dismiss the canvas — nodes remain visible and interactive behind it.

Panel sections:

**Node name** — editable text input. Changes the label on the canvas node in real time.

**Model** — dropdown selector. Options: Claude Sonnet 4, Claude Haiku 4.5, GPT-4o, Gemini 2.0 Flash, Gemini 2.5 Pro, OpenRouter (custom), Ollama (local). Selecting Ollama or OpenRouter reveals a secondary text input for the model string or local endpoint.

**System prompt** — full-height textarea showing the node's active system prompt. The default prompt is pre-filled per node type (see defaults above). The developer can:
- Read the full prompt
- Edit it directly in the textarea
- Save edits via the Save button

**AI prompt generation** — below the textarea, a single-line input labelled "Describe what this agent should do…" and an "AI generate ✦" button. When clicked:
1. The button enters a loading state ("Generating…")
2. Insomniac calls the configured AI provider (defaults to the project's primary model) with a meta-prompt: *"Write a concise system prompt (3–5 sentences) for an AI agent with the role: `{type}`. The user wants it to: `{seed}`. Output only the system prompt text."*
3. The returned text replaces the textarea content
4. A status line confirms: "Prompt generated. Review and save."
5. The developer reviews, edits if needed, and saves

The original default prompt is always recoverable via a "Reset to default" link below the textarea.

**Abilities** — tag list of Abilities assigned to this node. Each tag has a remove button. An "+ Add ability" button opens the Abilities catalogue filtered to installable items. Abilities assigned here define exactly what tools this agent can invoke — no ambient access.

**Edge condition** — dropdown setting the condition on outgoing edges from this node: Always continue / On success only / On failure only / On human decision. This sets the default for new edges drawn from this node's output port.

**Panel actions:**
- **Save changes** — writes all panel edits to the node definition and re-renders the canvas node
- **Delete** — removes the node and all connected edges with confirmation

#### Toolbar

Above the canvas:
- **+ Add node** — opens the Add Node type menu
- **Auto-layout** — arranges all nodes into a clean left-to-right grid (uses a simple column-row algorithm; ReactFlow's `dagre` layout for complex graphs)
- **Clear edges** — removes all edges, leaving nodes in place
- **Export JSON** — downloads the full chain definition as a `.insomniac-chain.json` file
- **Save as template** — saves the chain to the local template catalogue
- **Publish to Marketplace** — packages and submits to `insomniac-dev/store` via GitHub OAuth

#### Chain definition schema

The chain is serialised as JSON and stored in the project's SQLite record:
...
```json
{
  ...
  "nodes": [
    {
      ...
      "provider": "anthropic",
      "prompt": "You are a pipeline trigger node…",
      "abilities": [],
      "condition": "always",
      "x": 40,
      "y": 180
  ...
  "edges": [
    { "id": "e1", "from": "n1", "to": "n2", "type": "on-success" }
  ]
}
```

The pipeline execution engine reads this JSON and executes nodes in topological order, respecting edge conditions. Parallel branches (nodes with no data dependency) execute concurrently.
...
- **Visual editor overlay** — developer can:
  ...
  - Double-click text to edit it inline — edits reflected in the live preview and written back to the source file via source map resolution. **Writeback requires an unminified development build.** If a minified or production build is detected, the edit affordance is hidden and replaced with the notice: *"Source writeback requires an unminified development build."* (See resolved decision #9.)
...
| 7 | Lightpanda production readiness | A stability gate is defined before Lightpanda becomes the default on `app.insomniac.dev`: minimum Lightpanda v1.0.0 release, ≥95% pass rate on a curated corpus of 50 representative dev server UIs (covering Next.js, Vite, CRA, Astro, SvelteKit), and zero regressions on the Insomniac test suite. Until the gate is passed, the hosted version defaults to Playwright/Chromium. Self-hosted users may opt in to Lightpanda at any version via `browser.engine: "lightpanda"` in project config. |
| 8 | Firecracker on Windows self-hosted | Firecracker requires Linux KVM and does not run on Windows outside WSL2. This is a hosted-only concern — self-hosted users always run agents as local OS processes regardless of platform. On Windows self-hosted (native or WSL), no Firecracker involvement at any point. Documentation will include a clear platform matrix. If a user somehow sets `sandbox: "firecracker"` on a non-Linux self-hosted instance, the backend emits a startup error: *"Firecracker sandboxing requires Linux with KVM. On Windows, agents run as local processes. Remove the sandbox config or switch to hosted mode."* |
| 9 | Dev Browser source writeback scope | Source writeback (DOM edit → source file) is supported only for **unminified development builds** where source maps are present and resolve 1:1 to source files. Insomniac detects build mode by inspecting the dev server response headers and bundle output. If a minified or production build is detected, the visual editor overlay is available for inspection only — the "write back to source" affordance is hidden and replaced with a static notice: *"Source writeback requires an unminified development build. Start your dev server in development mode to enable editing."* The "Inspect in agent" action remains available regardless of build mode, as it sends a selector-based instruction rather than a file reference. |
...
All previously tracked open questions have been resolved. See §11 Resolved Decisions for the full record.

No open questions remain at v0.5. New questions should be logged here as they arise during implementation.
...
*Living document. No open questions remain at v0.5. New questions should be logged in §13 as they arise. Scope changes after M5 require a milestone review.*