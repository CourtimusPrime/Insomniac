...
import type { AgentAdapter, AgentConfig, AgentMessage, AgentStatus } from "./types.js";
...
export class McpAdapter implements AgentAdapter {
  ...
  private spawnProcess(): void {
    ...
    this.process.stderr?.on("data", (chunk: Buffer) => {
      console.error(`[McpAdapter][${this.config.name}] stderr: ${chunk.toString()}`);
    });
    ...
    this.process.on("exit", (code, signal) => {
      console.log(`[McpAdapter][${this.config.name}] process exited: code=${code} signal=${signal}`);
      this.process = null;
      this.initialized = false;
  ...
  private handleStdout(data: string): void {
    ...
    for (const line of lines) {
      ...
      try {
        ...
        console.warn(`[McpAdapter][${this.config.name}] non-JSON stdout: ${trimmed}`);
      }
    }
  }
  ...
  private attemptReconnect(): void {
    if (this.reconnectAttempts >= this.maxReconnectAttempts) {
      console.error(`[McpAdapter][${this.config.name}] max reconnect attempts reached`);
      this.status = "disconnected";
      this.drainResolvers({
        id: randomUUID(),
        type: "message",
    ...
    console.log(`[McpAdapter][${this.config.name}] reconnecting in ${delay}ms (attempt ${this.reconnectAttempts})`);

    setTimeout(() => {
      this.ensureConnection().catch((err) => {
        console.error(`[McpAdapter][${this.config.name}] reconnect failed:`, err);
        this.attemptReconnect();
      });
    }, delay);
  }
