...
import { SandboxManager, type SandboxConfig } from "../hosted/index.js";
  ...
  server.post<{ Body: CreateSandboxBody }>(
    ...
    async (request, reply) => {
      ...
      if (body.maxMemoryMB !== undefined) {
        if (typeof body.maxMemoryMB !== "number" || body.maxMemoryMB < 128 || body.maxMemoryMB > 4096) {
          reply.code(400).send({ error: "maxMemoryMB must be a number between 128 and 4096" });
          return;
        }
      }

      if (body.maxVCPUs !== undefined) {
        if (typeof body.maxVCPUs !== "number" || body.maxVCPUs < 1 || body.maxVCPUs > 8) {
          reply.code(400).send({ error: "maxVCPUs must be a number between 1 and 8" });
          return;
        }
      }
      ...
      if (body.networkMode !== undefined) {
        if (!["none", "nat", "bridged"].includes(body.networkMode)) {
          reply.code(400).send({ error: "networkMode must be one of: none, nat, bridged" });
          return;
        }
      }

      const overrides: Partial<SandboxConfig> = {};
      if (body.maxMemoryMB !== undefined) overrides.maxMemoryMB = body.maxMemoryMB;
      if (body.maxVCPUs !== undefined) overrides.maxVCPUs = body.maxVCPUs;
      if (body.networkMode !== undefined) overrides.networkMode = body.networkMode;
